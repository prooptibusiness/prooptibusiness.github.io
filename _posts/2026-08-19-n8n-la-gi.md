---
layout: post
title: "n8n là gì? Workflow Automation, Self-host và ứng dụng cho Marketing"
slug: n8n-la-gi
description: "n8n là gì? Hiểu Workflow Automation theo Node, Trigger, Credential, API, Self-host và các Use Case Marketing từ Lead Routing đến báo cáo."
answer_summary: "n8n là nền tảng Workflow Automation cho phép kết nối ứng dụng, API và Logic bằng các Node trong một Workflow. n8n có Cloud và lựa chọn Self-host tùy nhu cầu. Với Marketing, nó phù hợp cho Lead Routing, Sync CRM, Content Operations, Reporting và AI Workflow; cần quản Credential, Retry, Logging và quyền truy cập cẩn thận."
target_query: "n8n là gì?"
...
tags: ["n8n","Workflow Automation","API","MarTech"]
image: /assets/blog/n8n-la-gi-2026.svg
image_alt: "Minh họa 2D n8n là gì theo phong cách biểu đồ và sơ đồ PoB"
...
published: true
faq:
  - question: "n8n có phải No-code không?"
    answer: "Nó hỗ trợ Visual Workflow nhưng nhiều Use Case nâng cao vẫn hưởng lợi từ hiểu API, JSON và Code."
  - question: "n8n có Self-host được không?"
    answer: "Có theo tài liệu n8n; Self-host đòi hỏi năng lực vận hành và bảo mật."
  - question: "n8n có dùng cho AI Workflow được không?"
    answer: "Có thể kết nối Model, API và Tool, nhưng cần Guardrail và Cost Monitoring."
  - question: "n8n khác Zapier hoặc Make thế nào?"
    answer: "Khác về trải nghiệm, Hosting, Connector, Pricing và mức kỹ thuật; chọn theo Use Case."
---

## n8n là gì?

**n8n là gì** nên được hiểu như một mô hình làm việc có thể đo và cải tiến, không phải một nhãn thời thượng. Với PoB, giá trị của chủ đề này nằm ở chỗ nó giúp đội ngũ liên kết hành vi khách hàng, dữ liệu và quyết định vận hành thay vì chỉ nhìn một chỉ số riêng lẻ.
...
Một mô hình chỉ có giá trị khi nó giúp thay đổi quyết định. Với **n8n là gì**, đội ngũ nên hỏi ba lớp câu hỏi: vấn đề nào đang xảy ra, tín hiệu nào chứng minh vấn đề đó, và thay đổi nào có khả năng tạo tác động đủ lớn. Cách tiếp cận này tránh việc chọn giải pháp trước rồi mới đi tìm lý do.
...
Webhook, Schedule hoặc Event tùy Node. Đây là một phần cần được định nghĩa bằng hành vi hoặc dữ liệu cụ thể để cả team hiểu giống nhau.

### Credentials

Thông tin xác thực tới App hoặc API. Đây là một phần cần được định nghĩa bằng hành vi hoặc dữ liệu cụ thể để cả team hiểu giống nhau.

### Data Mapping

Chuyển Field giữa các Node. Đây là một phần cần được định nghĩa bằng hành vi hoặc dữ liệu cụ thể để cả team hiểu giống nhau.

### Branch/Loop

Logic điều kiện và xử lý nhiều Item. Đây là một phần cần được định nghĩa bằng hành vi hoặc dữ liệu cụ thể để cả team hiểu giống nhau.

### Error Workflow

Cơ chế Catch, Alert hoặc Retry cho Failure. Đây là một phần cần được định nghĩa bằng hành vi hoặc dữ liệu cụ thể để cả team hiểu giống nhau.
...
1. **Bắt đầu bằng Workflow Low-risk như Report Notification.**
2. **Dùng Test Data và Sample Data cẩn thận.**
3. **Tách Credential theo môi trường hoặc Service Account nếu có.**
4. **Thiết kế Idempotency khi Workflow tạo hoặc ghi Record.**
5. **Bật Monitoring và Review Execution Error định kỳ.**
...
| Chỉ số | Cách đọc |
|---|---|
| Execution Success Rate | Tỷ lệ Run thành công. |
| Latency | Thời gian từ Trigger đến Completion. |
| Manual Time Saved | Giá trị vận hành tiết kiệm. |
| Error Recovery Time | Thời gian phát hiện và xử lý lỗi. |
| Cost per Execution | Chi phí Cloud hoặc hạ tầng theo Usage. |
...
- **Self-host mà không Patch hoặc Update.**
- **Lưu Secret trong Code hoặc Plain Text.**
- **Không có Duplicate Protection.**
- **Workflow quá lớn không chia Module.**
- **Không có Alert nên lỗi âm thầm nhiều ngày.**
...
- **Nguồn:** [n8n — Hosting](https://docs.n8n.io/hosting/)
- **Nguồn:** [n8n — Error Handling](https://docs.n8n.io/flow-logic/error-handling/)
