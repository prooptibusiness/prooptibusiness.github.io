---
layout: post
title: "Webhook là gì? Cách nhận sự kiện Real-time cho Automation"
slug: webhook-la-gi
description: "Webhook là gì? Hiểu Event, Endpoint, Payload, Signature, Retry, Idempotency và cách Webhook kích hoạt Workflow Marketing gần thời gian thực."
answer_summary: "Webhook là cơ chế để một hệ thống chủ động gửi HTTP Request tới Endpoint của hệ thống khác khi có sự kiện, ví dụ Form mới, Payment hoàn tất hoặc Order cập nhật. Webhook giúp giảm Polling và phản ứng gần thời gian thực, nhưng Endpoint phải xác thực Signature, xử lý Retry, Duplicate, Timeout và lưu Log."
...
tags: ["Webhook","Automation","API","Integration"]
image: /assets/blog/webhook-la-gi-2026.svg
image_alt: "Minh họa 2D Webhook là gì theo phong cách biểu đồ và sơ đồ PoB"
...
published: true
faq:
  - question: "Webhook có phải API không?"
    answer: "Webhook thường dùng HTTP API Pattern nhưng vai trò là Event Push từ Source sang Consumer."
  - question: "Webhook có luôn Real-time không?"
    answer: "Thường gần Real-time, nhưng có thể có Queue, Retry hoặc Delay."
  - question: "Tại sao Webhook gửi trùng?"
    answer: "Retry là hành vi bình thường khi Source không chắc Consumer đã xử lý; cần Idempotency."
  - question: "Có cần Public Endpoint không?"
    answer: "Thường Source cần truy cập Endpoint qua Internet, trừ kiến trúc Private đặc thù."
---

## Webhook là gì?

**Webhook là gì** nên được hiểu như một mô hình làm việc có thể đo và cải tiến, không phải một nhãn thời thượng. Với PoB, giá trị của chủ đề này nằm ở chỗ nó giúp đội ngũ liên kết hành vi khách hàng, dữ liệu và quyết định vận hành thay vì chỉ nhìn một chỉ số riêng lẻ.
...
Một mô hình chỉ có giá trị khi nó giúp thay đổi quyết định. Với **Webhook là gì**, đội ngũ nên hỏi ba lớp câu hỏi: vấn đề nào đang xảy ra, tín hiệu nào chứng minh vấn đề đó, và thay đổi nào có khả năng tạo tác động đủ lớn. Cách tiếp cận này tránh việc chọn giải pháp trước rồi mới đi tìm lý do.
...
Xác định sự kiện nào đã xảy ra. Đây là một phần cần được định nghĩa bằng hành vi hoặc dữ liệu cụ thể để cả team hiểu giống nhau.

### Endpoint

URL nhận Webhook. Đây là một phần cần được định nghĩa bằng hành vi hoặc dữ liệu cụ thể để cả team hiểu giống nhau.

### Payload

Thông tin về Event. Đây là một phần cần được định nghĩa bằng hành vi hoặc dữ liệu cụ thể để cả team hiểu giống nhau.

### Signature

Cơ chế xác minh Request thật sự từ Source. Đây là một phần cần được định nghĩa bằng hành vi hoặc dữ liệu cụ thể để cả team hiểu giống nhau.

### Idempotency

Xử lý cùng Event nhiều lần mà không tạo hậu quả trùng. Đây là một phần cần được định nghĩa bằng hành vi hoặc dữ liệu cụ thể để cả team hiểu giống nhau.
...
1. **Đọc tài liệu Webhook của Source và Signature Method.**
2. **Tạo Endpoint HTTPS bảo mật.**
3. **Trả Response nhanh rồi xử lý Async nếu tác vụ nặng.**
4. **Lưu Event ID để chống Duplicate.**
5. **Thiết kế Replay hoặc Manual Retry cho Event lỗi.**
...
| Chỉ số | Cách đọc |
|---|---|
| Delivery Success | Tỷ lệ Webhook được nhận và xử lý. |
| End-to-End Latency | Thời gian từ Event đến Action. |
| Duplicate Rate | Tỷ lệ Event lặp. |
| Retry Volume | Số lần Source Retry. |
| Dead-letter Volume | Sự kiện không xử lý được cần can thiệp. |
...
- **Tin Payload mà không Verify Signature.**
- **Xử lý tác vụ nặng trước khi trả Response.**
- **Không chống Duplicate.**
- **Không lưu Log tối thiểu phù hợp Privacy.**
- **Không Rotate Secret.**
...
- **Đọc tiếp:** [Make.com](/blog/make-com-la-gi/) — Nối chủ đề này với hệ thống kiến thức PoB.
...
- **Nguồn:** [Stripe — Webhooks](https://docs.stripe.com/webhooks)
- **Nguồn:** [GitHub — Webhooks](https://docs.github.com/en/webhooks)
